      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button
            ...
            aria-label={
              gap > 0
                ? `Generate ${gap} missing ${NodeTypeDisplayNames[nodeType]} node${gap !== 1 ? 's' : ''} for ${BiomeDisplayNames[biome]}`
                : `${NodeTypeDisplayNames[nodeType]} in ${BiomeDisplayNames[biome]}: ${actual} of ${target}`
            }
          >
...
export function DistributionGapChart({ onSelectGap }: DistributionGapChartProps) {
  ...
  const matrix = useMemo(() => {
    ...
    const result = {} as Record<Biome, Record<NodeType, { target: number; actual: number }>>;
