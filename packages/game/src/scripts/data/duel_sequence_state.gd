...
func reset() -> void:
	...
	show_rewards = false
	is_active = false
	GLog.debug("Test sequence state reset", "duel_sequence")
...
func get_next_enemy() -> Resource:
	...
	if is_sequence_complete():
		GLog.warn("Sequence complete, no more enemies", "duel_sequence")
		return null

	if current_enemy_index < 0 or current_enemy_index >= enemy_queue.size():
		GLog.error("Invalid enemy index: %d / %d" % [current_enemy_index, enemy_queue.size()], "duel_sequence")
		return null
...
func advance_to_next_enemy() -> void:
	...
	GLog.debug("Advanced to enemy %d/%d" % [current_enemy_index + 1, total_enemies], "duel_sequence")
