...
## Implementation Snapshot (data/characters/prospector.tres)

- mechanical_specialization: Fortune
- base_health: 45, base_sanity: 110, base_energy: 3, starting_gold: 25
- starting_deck: 7 entries (resources), starting_deck_size: 7
- passive_abilities: ["Gold Rush", "Claim Bonus", "Prospector's Eye"]
- active_abilities: ["Risk Tolerance"]
- unique_resources: ["Fortune Streak"]
- preferred_card_types: ["Gamble"], forbidden_card_types: []

## Deltas vs Spec

- Uses `starting_deck` (resource refs) rather than `starting_deck_paths` (string paths). Consider normalizing format across classes later.
...
3. Risk Tolerance ability implementation
4. Balance testing with existing enemies
