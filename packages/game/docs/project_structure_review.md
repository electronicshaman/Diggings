# Project Structure Review & Godot 4.4 Best Practices Recommendations
     ...
     │   ├── audio/
     │   └── fonts/
     ├── data/                           # RENAMED: resources → data  
     │   ├── cards/                      # MOVED: test_cards → data/cards
     │   │   ├── attack/                 # RENAMED: gold → attack (theme-agnostic)
     │   │   ├── skill/                  # RENAMED: grit → skill
     │   │   ├── power/                  # RENAMED: grog → power
     │   │   └── fortune/                # RENAMED: gamble → fortune
     │   ├── enemies/                    # MOVED: test_enemies → data/enemies
     │   ├── characters/                 # NEW: Character class definitions
     │   └── themes/                     # NEW: Theme configuration files
     ├── scenes/                         # Keep existing
     │   ├── ui/                         # NEW: Organize UI scenes
     │   ├── game/                       # NEW: Game scenes
     │   └── cards/                      # NEW: Card-related scenes
     ├── scripts/                        # Keep existing structure, minor improvements
     │   ├── autoloads/                  # NEW: Dedicated autoload scripts
     │   ├── components/                 # NEW: Reusable components
     │   ├── managers/                   # MOVED: combat → managers
     │   └── systems/                    # NEW: Game systems
     └── docs/                          # Keep as-is - excellent structure

     2. Autoload System Optimization

     Current Issues:

     - Inconsistent autoload naming
     - Missing GameManager/SceneManager
     - No centralized state management
     - Loading order dependencies not considered

     Recommended Autoloads (in load order):
     ...
     This reorganization will create a more maintainable, scalable, and performance-optimized project structure that follows Godot 4.4 best practices while preserving the excellent 
