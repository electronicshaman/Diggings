...
func _select_balanced(enemy: EnemyState, playable: Array[CardData]) -> CardData:
	...
	var player_health_ratio = duel_state.player_data.stats.get_health_percentage() if duel_state.player_data and duel_state.player_data.stats else 1.0
	var enemy_health_ratio = enemy.stats.get_health_percentage() if enemy.stats else 1.0
