...
func add_curio(curio: Resource) -> bool:
	...
	var corruption_cost = curio.get("corruption_cost", 0)
...
func remove_curio(curio: Resource) -> void:
	...
	GLog.debug("Removing curio: " + curio.get("curio_name", "Unknown"))
	
	# Handle stacked curios
	var curio_name = curio.get("curio_name", "")
	if curio.get("stackable", false) and curio_stacks.has(curio_name):
		var current_stacks = curio_stacks[curio_name]
		if current_stacks > 1:
			curio_stacks[curio_name] = current_stacks - 1
			curio_stack_changed.emit(curio, current_stacks - 1)
...
func trigger_curio_effects(event_type: String, context: Dictionary = {}) -> void:
	...
	for curio in active_curios:
		var effects = curio.get("effects", [])
		for effect in effects:
			if effect and effect.get("trigger_event") == event_type:
				if effect.can_trigger(self, context):
					var effect_name = effect.get("effect_name", "Unknown")
					var curio_name = curio.get("curio_name", "Unknown")
...
func _on_card_played(card: Node) -> void:
	var context = {
		"card": card,
		"card_data": card.get("card_data") if card.has_method("get") else null
	}
...
func _on_damage_taken(target: Node, amount: int) -> void:
	# Only trigger if damage was to player
...
func get_save_data() -> Dictionary:
	...
	for curio in active_curios:
		var resource_path = curio.get("resource_path", "")
		if resource_path:
...
func debug_list_curios() -> void:
	...
	for curio in active_curios:
		...
		var rarity = curio.get("rarity", "Common")
		var description = curio.get("description", "")
