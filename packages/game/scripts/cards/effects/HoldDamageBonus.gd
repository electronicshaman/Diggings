...
const EFFECT_NAME := "Hold Damage Bonus"
...
func apply_effect(_duel_manager: Node, card_data: Resource, results: Dictionary) -> void:
	...
	print("Applied %s effect from %s (+%d damage to %s cards)" % [get_effect_name(), card_data.card_name, bonus_damage, card_type_filter if card_type_filter != "" else "all"])
