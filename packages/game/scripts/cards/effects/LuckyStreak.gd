...
const EFFECT_NAME := "Lucky Streak"
...
func apply_effect(_duel_manager: Node, card_data: Resource, results: Dictionary) -> void:
	...
	print("Applied %s effect from %s (%.1f per %s, max %.1f)" % [get_effect_name(), card_data.card_name, luck_bonus_per_power, card_type_trigger, maximum_bonus])
